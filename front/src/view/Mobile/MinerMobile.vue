        ...
        <div class="con_table">
          <div class="trans_ul">
            <li v-for="(item, index) of dataList" :key="index">
              <p>
                <span>{{ $t("miner.name") }}</span
                >{{ item.minerName }}
                {{
                  item.lvl === 1 ? "(" + $t("miner.officialMiner") + ")" : ""
                }}
              </p>
              <p>
                <span>{{ $t("miner.weights") }}</span
                >{{ item.pledgeWeight }}
              </p>
              <p>
                <span>{{ $t("miner.numberBlocks") }}</span
                >{{ item.totalProduced }}
              </p>
              <p>
                <span>{{ $t("miner.missed") }}</span
                >{{ item.totalMissed }}
              </p>
              <p>
                <span>{{ $t("miner.lastTime") }}</span
                >{{ item.lastConfirmedBlockNum }}
              </p>
              <p>
                <span>{{ $t("miner.fee") }}</span
                >{{ item.minerPledgePayBack + "%" }}
              </p>
            </li>
          </div>
