      ...
      <div class="con_all">
        ...
        <div class="all_aside">
              ...
              <el-table :data="tokenTransactions" style="width: 100%">
                ...
                <el-table-column align="center" label="Amount">
                  <template slot-scope="scope">
                    <span
                      >{{ scope.row.amountStr }} &nbsp;
                      {{ scope.row.symbol }}</span
                    >
                  </template>
                </el-table-column>
                <el-table-column align="center" label="Memo">
                  <template slot-scope="scope">
          ...
          <template v-if="choiceFlag === 3">
            <div class="table-wrap">
              <el-table :data="balancesData" style="width: 100%">
                <el-table-column
                  align="center"
                  type="index"
                  width="120"
                ...
                >
                  <template slot-scope="scope">
                    <span
                      v-if="
                        scope.row.addr ===
                        'XWCNVjD93CFH7SFjLquke2BL9nrPN4Vvkd5Hi'
                      "
                      class="link"
                      @click="_mixin_address_jump(scope.row.addr)"
                    >
                      {{ scope.row.addr }}
                      <font>({{ $t("richlist.teamHold") }})</font></span
                    >
                    <span
                      v-else-if="
                        scope.row.addr ===
                        'XWCNYzg8sUkoy2kFbzvj6xSky7Hm7RuiTPFqr'
                      "
                      class="link"
                      @click="_mixin_address_jump(scope.row.addr)"
                    >
                      {{ scope.row.addr
                      }}<font>({{ $t("richlist.oldExchange") }})</font></span
                    >
                    <span
                      v-else
                      class="link"
...
export default {
  ...
  methods: {
    ...
    getContractInfo() {
      let that = this;
      this.$axios
        .post("/getContractStatis", { contractId: this.contractAddress })
        .then(function (res) {
    ...
    getAbiData() {
      let that = this;
      this.$axios
        .post("/getAbi", { contractId: this.contractAddress })
        .then(function (res) {
          if (res.data.retCode === 200 && res.data.data !== null) {
